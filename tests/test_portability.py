...
class PortabilityTests(unittest.TestCase):
    def test_no_serial_device_is_opened_by_default(self):
        ...
        with patch.dict(os.environ, {}, clear=True), patch.dict(sys.modules, {
            ...
            module = load_file("nose_test", "src/mediapipe_gesture_recognition/nose_cam.py")
    ...
    def test_disabled_lights_do_not_send_commands(self):
        import mediapipe_gesture_recognition
        plugs = Mock()
        with patch.dict(sys.modules, {"mediapipe_gesture_recognition.tp_link": plugs}), patch.object(
            mediapipe_gesture_recognition, "tp_link", plugs, create=True
        ), patch.dict(os.environ, {"ENABLE_SMART_LIGHTS": "false"}):
            module = load_file("lights_test", "src/mediapipe_gesture_recognition/light_logic.py")
    ...
    def test_scene_images_are_rgb_and_missing_frame_skips_api(self):
        ...
        with patch.dict(sys.modules, {
            ...
            "mediapipe_gesture_recognition.AI": fake_ai,
            "google.genai": fake_genai,
        }), patch("google.genai", fake_genai):
            module = load_file("mediapipe_gesture_recognition.scene_test", "src/mediapipe_gesture_recognition/scene_monitor.py")
    ...
    def test_launcher_checks_keys_before_importing_application(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            (root / "src/mediapipe_gesture_recognition").mkdir(parents=True)
            (root / "src/mediapipe_gesture_recognition/gesture_recognizer.task").touch()
            try:
